...
func Handle(path []string, as *app.State, ps *cutil.PageState) (string, layout.Page, []string, error) {
	...
	case util.AppKey:
		msg := "\n  " +
			"<meta name=\"go-import\" content=\"github.com/kyleu/solitaire git %s\">\n  " +
			"<meta name=\"go-source\" content=\"github.com/kyleu/solitaire %s %s/tree/master{/dir} %s/blob/master{/dir}/{file}#L{line}\">"
	...
	case keyDownload:
		...
		ps.Title = "Downloads"
		ps.Data = util.ValueMap{"base": "https://github.com/kyleu/solitaire/releases/download/v" + as.BuildInfo.Version, "links": dls}
