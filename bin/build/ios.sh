...
##
## Outputs:
##   - build/dist/solitaire_<version>_ios_framework.zip
##   - build/dist/solitaire_<version>_ios_app.zip
...
GOARCH=arm64 time gomobile bind -o build/dist/mobile_ios_arm64/solitaireServer.xcframework -target=ios github.com/kyleu/solitaire/app/cmd
echo "gomobile for iOS completed successfully, building distribution..."
cd "build/dist/mobile_ios_arm64/solitaireServer.xcframework"
zip --symlinks -r "../../solitaire_${TGT}_ios_framework.zip" .
...
xcodebuild -project "Solitaire.xcodeproj" -allowProvisioningUpdates
zip -r "$dir/../../build/dist/solitaire_${TGT}_ios_app.zip" .

cd "$dir/../.."
echo "Output written to ./build/dist/solitaire_${TGT}_ios_framework.zip and ./build/dist/solitaire_${TGT}_ios_app.zip"
